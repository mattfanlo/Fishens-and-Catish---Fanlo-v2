class GameScene2 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('bg2', 'assets/images/bg_level2.jpg');
        this.load.image('tiles2', 'assets/images/platform2.png');
    ...
    create() {
        ...
        this.add.image(0, 0, 'bg2').setOrigin(0, 0).setDisplaySize(config.width, config.height);
        ...
        const ground = this.add.tileSprite(config.width / 2, config.height - 16, config.width, 32, 'tiles2');
        this.physics.add.existing(ground, true);
        this.platforms.add(ground);

        const positions = [
            [200, 520], [600, 420], [1000, 320],
            [400, 250], [800, 180], [1200, 500]
        ];
        positions.forEach(([x, y]) => {
            const platform = this.add.tileSprite(x, y, 250, 32, 'tiles2');
        ...
        this.coins = this.physics.add.group({ key: 'coin', repeat: 14 });
    ...
    collectCoin(player, coin) {
        ...
        this.scoreLabel.setText('Score: ' + score+'/300');
    ...
    winCondition() {
        ...
        const winText = this.add.text(config.width / 2, config.height / 2 - 50, 'You beat Level 2', {
            fontSize: '64px',
            fill: '#00ffff',
            stroke: '#000000',
            strokeThickness: 6
        ...
        const proceedText = this.add.text(config.width / 2, config.height / 2 + 10, 'Click anywhere to proceed to Level 3', {
            fontSize: '32px',
            fill: '#ffffff'
        }).setOrigin(0.5);
